...
class ContactInboxBuilder
  ...
  def generate_source_id
    ...
    when 'Channel::TwilioSms'
      twilio_source_id
    when 'Channel::Whatsapp', 'Channel::Evolution', 'Channel::EvolutionGo'
      wa_source_id
    when 'Channel::Email'
      email_source_id
    when 'Channel::Sms'
    ...
    when 'Channel::Instagram'
      instagram_source_id
    when 'Channel::Api', 'Channel::WebWidget', 'Channel::FacebookPage'
      SecureRandom.uuid
    else
  ...
  def instagram_source_id
    # source_id should always be provided explicitly for Instagram channels
    # This method should never be called in practice, but if it is, we raise an error
    raise ActionController::ParameterMissing, 'Instagram user ID (source_id) must be provided when creating contact_inbox for Instagram channel'
  end

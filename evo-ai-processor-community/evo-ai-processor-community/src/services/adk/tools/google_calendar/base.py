...
class GoogleCalendarClient:
    ...
    def __init__(self, db=None):
        ...
        """
        self.db = db
        # NOTE: Intentionally no in-memory cache here.
        # Caching credentials caused stale (expired) tokens to persist after
        # the user reconnected their Google Calendar account. Each call fetches
        # fresh credentials directly from the database.
    ...
    async def get_integration(
        ...
        Always reads fresh from DB — no in-memory caching — so reconnects take effect immediately.
        ...
        """
        # Load directly from database (no sanitization, no cache)
    ...
    def get_calendar_service(self, credentials_dict: Dict[str, Any]):
        ...
        Auto-refreshes short-lived access tokens using the stored refresh_token.
        ...
        # If the access_token is expired but we have a refresh_token, refresh silently
        if credentials.expired and credentials.refresh_token:
            try:
                credentials.refresh(Request())
                logger.info("[GoogleCalendar] Access token refreshed successfully")
            except Exception as refresh_err:
                logger.warning(f"[GoogleCalendar] Token refresh failed: {refresh_err}")
                raise

...
class RunnerUtils:
    ...
    async def get_or_create_session(
        ...
        # Sync to database if it was not found initially or newly created
        # We know it wasn't found in DB if we reached here because get_session_by_id didn't return it
        try:
            from src.models.models import Session as SessionModel
            from sqlalchemy.exc import IntegrityError
            
            # Check if it exists to be safe
            existing = self.db.query(SessionModel).filter(SessionModel.id == adk_session_id).first()
            if not existing:
                ...
                self.db.commit()
                logger.info(f"✅ Synced session {adk_session_id} to evo_ai_agent_processor_sessions")
        except IntegrityError:
            self.db.rollback()
        except Exception as e:
            self.db.rollback()
            logger.error(f"Failed to sync session {adk_session_id} to database: {e}")
    ...
    async def setup_session_state(
        ...
        # Combine all state updates into a single dictionary
        state_changes = {}
            ...
            if conversation_id:
                try:
                    ...
                    if result:
                        ...
                        if summary:
                            state_changes["handoff_summary"] = summary
                            state_changes["handoff_entities"] = entities
                            logger.info(f"[RunnerUtils] Injected handoff context into state for conversation {conversation_id}")
                except Exception as e:
                    logger.error(f"[RunnerUtils] Error fetching handoff context: {e}")

        # Create a single consolidated event for all state updates
        ...
        await session_service.append_event(session, consolidated_event)
        logger.info(f"[RunnerUtils] ✅ Consolidated all state and metadata into a single ADK event to prevent context pollution")
        ...
        if files and len(files) > 0:
            for file_data in files:
                try:
                    ...
                    is_document = self._is_document_file(file_data.content_type, file_data.filename)
                    if is_audio or is_image or is_video or is_document:
                        # Add to content parts for LLM processing
    ...
    def _is_document_file(self, content_type: str, filename: str) -> bool:
        """Check if file is a document (PDF, text, CSV)."""
        if not content_type and not filename:
            return False

        if content_type:
            main_mime_type = content_type.split(";")[0].strip().lower()
            if main_mime_type in ["application/pdf", "text/plain", "text/csv"]:
                return True

        if filename:
            doc_extensions = [".pdf", ".txt", ".csv", ".md"]
            filename_lower = filename.lower()

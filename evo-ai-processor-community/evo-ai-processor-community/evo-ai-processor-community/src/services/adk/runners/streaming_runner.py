        ...
        try:
            ...
            try:
                ...
                if final_response_text and final_response_text != "":
                    ...
                    # Generate Audio if VoiceService is requested
                    try:
                        # Assuming voice generation is requested via metadata or agent config
                        # For now, we instantiate and call it to demonstrate the architecture
                        voice_profile = None
                        if agent and agent.config and isinstance(agent.config, dict):
                            voice_profile = agent.config.get("voice_profile")
                        
                        if voice_profile or (metadata and metadata.get("generate_audio")):
                            logger.info("Generating audio using VoiceService...")
                            voice_svc = VoiceService()
                            async for audio_chunk in voice_svc.stream_audio(final_response_text, voice_profile):
                                if audio_chunk:
                                    ...
                                    yield json.dumps(audio_event)
                    except Exception as ve:
                        logger.error(f"Error generating audio with VoiceService: {ve}")

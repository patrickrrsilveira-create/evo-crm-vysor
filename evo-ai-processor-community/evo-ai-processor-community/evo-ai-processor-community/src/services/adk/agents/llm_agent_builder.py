...
class LlmAgentBuilder:
    ...
    async def _create_llm_agent(
        ...
        if tts_config and tts_config.get("apiKey") or agent.config.get("voice_profile"):
            # Ensure respond_in_audio defaults gracefully
            respond_in_audio = tts_config.get("respondInAudio", "when_client_asks") if tts_config else "when_client_asks"
            
            voice_instructions = [
                "VOCÊ ESTÁ FALANDO, NÃO ESCREVENDO. Seu texto será convertido em áudio, então siga estas regras estritamente:",
                "1. Escreva EXATAMENTE como se fala. Use pausas naturais (vírgulas) e evite pontos de exclamação exagerados.",
                "2. Adicione hesitações humanas naturais (ah, hmm, é...) no início de frases pensativas.",
                "3. Escreva números por extenso (ex: 'vinte e dois' ao invés de '22').",
                "4. NUNCA use emojis, asteriscos, hashtags ou símbolos especiais (*, #, $, @). Eles soam como robôs lendo símbolos.",
                "5. Use contrações comuns do português falado (ex: 'pra' em vez de 'para', 'tá' em vez de 'está').",
                "6. Seja breve e direto. Textos curtos e interativos são melhores para áudio.",
                "7. NUNCA mencione links, URLs ou diga 'você pode ouvir aqui'. Apenas fale normalmente.",
            ]
            
            if respond_in_audio == "always":
                crm_tools_instructions.append(
                    "text_to_speech Tool: You MUST use the `text_to_speech` tool on EVERY response. "
                    "Pass the EXACT text of your response to the tool to generate the audio version. "
                    "DO NOT answer only with text. NEVER append audio links or URLs to your response text.\n\n"
                    "INSTRUÇÕES DE VOZ (OBRIGATÓRIO):\n" + "\n".join(voice_instructions)
                )
            elif respond_in_audio == "when_client_asks":
                crm_tools_instructions.append(
                    "text_to_speech Tool: Available. Use the `text_to_speech` tool to generate an audio version of your response "
                    "ONLY when the user explicitly requests an audio response or sends a voice message. "
                    "NEVER append audio links or URLs to your response text.\n\n"
                    "INSTRUÇÕES DE VOZ (APLICAR SOMENTE SE FOR RESPONDER EM ÁUDIO):\n" + "\n".join(voice_instructions)
                )

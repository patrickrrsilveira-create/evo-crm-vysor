            ...
            if "bytes" in file_data and file_data["bytes"]:
                ...
                try:
                    ...
                    if "/rails/active_storage/" in url:
                        ...
                        logger.info(f"🚀 [HAIRPIN NAT FIX] Rewrote active_storage URL to: {url[:80]}...")
                    ...
                    async with httpx.AsyncClient(timeout=30.0, follow_redirects=False) as client:
                        ...
                        while response.is_redirect and redirects < max_redirects:
                            ...
                            if "/rails/active_storage/" in location:
                                ...
                                logger.info(f"🚀 [HAIRPIN NAT FIX] Rewrote redirect location to: {location[:80]}...")
...
async def extract_conversation_history(
    ...
    try:
        ...
        try:
            ...
            if not session:
                logger.info(f"ℹ️ Session not found (new conversation): {session_id}")
                return []
...
async def handle_message_send(
    ...
    try:
        ...
        if match:
            ...
            final_response = re.sub(transfer_pattern, '', final_response, flags=re.IGNORECASE).strip()
            result["final_response"] = final_response

        # Keep the old regex just in case it still uses the tool_call syntax sometimes
        pattern = r'<tool_call:transfer_conversation\{(.*?)\}>'
        match_tool = re.search(pattern, final_response)
        if match_tool:
            args_str = match_tool.group(1)
            args = {}
            arg_pattern = r'(\w+):<\|"\|>(.*?)<\|"\|>'
        ...
        # Strip audio URL lines the LLM may have appended to its text response.
        # Pattern: "Você pode ouvir minha mensagem [aqui](https://...)" or bare minio/audio URLs.
        # These should never appear in the text sent to the client.
        final_response = re.sub(r'\[([^\]]+)\]\(https?://[^)]+\)', r'\1', final_response)  # [text](url) → text
        final_response = re.sub(r'https?://\S+', '', final_response)  # bare URLs
        # Remove leftover phrases like "Você pode ouvir minha mensagem aqui." that become orphaned
        final_response = re.sub(r'Você pode ouvir (minha mensagem|o áudio)[^.]*\.?', '', final_response, flags=re.IGNORECASE)

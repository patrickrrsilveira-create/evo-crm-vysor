      ...
      Promise.all([fetchGlobalConfig(), fetchSetupStatus()]).then(([configData, isSetupRequired]) => {
        if (mounted) {
          ...
          if (configData.appPrimaryColor) {
            // Tailwind v4 and Shadcn compatibility: 
            // set --primary and --color-primary
            document.documentElement.style.setProperty('--primary', configData.appPrimaryColor);
            document.documentElement.style.setProperty('--color-primary', configData.appPrimaryColor);
          }
        }
      });
    };

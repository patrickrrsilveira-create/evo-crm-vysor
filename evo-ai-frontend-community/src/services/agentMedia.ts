...
export const agentMediaService = {
  list: async (agentId: string): Promise<AgentMedia[]> => {
    try {
      const response = await agentProcessorApi.get(`/adk/agent_media/${agentId}/media`);
  ...
  upload: async (agentId: string, file: File) => {
    try {
      ...
      const response = await agentProcessorApi.post(`/adk/agent_media/${agentId}/media`, formData, {
        headers: {
  ...
  delete: async (agentId: string, filename: string) => {
    try {
      const response = await agentProcessorApi.delete(`/adk/agent_media/${agentId}/media/${filename}`);
      return response.data;
    } catch (error) {
